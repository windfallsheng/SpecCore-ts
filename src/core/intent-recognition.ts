...
const COMMAND_MAPPINGS: CommandMapping[] = [
  ...
  // 创建迭代（必须在通用创建之前，优先级更高）
  {
    ...
    priority: 92,
    triggers: ['创建迭代', '新建迭代', '创建sprint', '新建sprint', '开新迭代', '新开迭代'],
    patterns: ['创建(.+)迭代', '新建(.+)迭代', '创建一个叫(.+)的迭代', '创建sprint(.+)', '开(.+)迭代', '迭代叫(.+)'],
  ...
  // 需求分析（全局级 + 迭代级 + 任务级 + 重新/补充分析）
  {
    ...
    triggers: [
      ...
      '按需分析', '只分析', '仅分析', '过滤分析',
      '聚焦分析', '限定分析', '针对分析',
      // v7.2.0+: 代码关联 — --with-code
      '带代码分析', '连代码分析', '源码分析', '结构化分析',
    ],
    patterns: [
      '分析(.+)需求', '分析(.+)代码',
      '迭代(.+)分析', '本迭代(.+)分析',
      ...
      '分析(.+)中的(.+)', '分析(.+)的(.+)', '深入(.+)的(.+)',
      '查看(.+)中的(.+)', '查看(.+)的(.+)',
      // v7.2.0+: 深度分析模式 — 指定文档 + 深度意愿
      '深度分析(.+)', '深入分析(.+)', '详细分析(.+)',
      '迭代分析(.+)', '逐节分析(.+)', '大纲分析(.+)',
      // v7.2.0+: 按需分析模式 — 指定模块/关键词
      '只分析(.+)', '仅分析(.+)', '过滤分析(.+)',
      '聚焦(.+)分析', '针对(.+)分析',
      // v7.2.0+: 全局 + 深度组合
      '全局深度分析(.+)', '全量深入分析(.+)',
    ...
    description: 'AI 分析 — 需求完整性 + 改动范围 + 风险矩阵（支持全局级/迭代级/任务级/重新分析/深度分析/按需分析/代码关联）',
    args: '-I <iteration> 或 --scope global|task --deep <doc> --iterative --filter <keywords> --with-code --supplement --sync',
  },
  // 任务拆分
  {
    id: 'iteration split',
