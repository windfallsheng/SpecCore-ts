...
export function buildNewPlatformPrompt(
  ...
  prompt += `- Read \`.speccore/GLOBAL/FUNCTION_MAP.md\` → 了解已有功能单元\n`;
  prompt += `- Read \`.speccore/GLOBAL/API_CONTRACT.yaml\` → 了解已有接口契约\n`;
  prompt += `- Read \`.speccore/GLOBAL/ARCHITECTURE.md\` → 了解全局架构\n`;
