...
function buildSplitInstruction(): string {
  return [
    ...
    '**绝对禁止拆出超过 20 个任务！**',
    '- 理想范围: **3-12 个任务/迭代**',
    '- 最大上限: **20 个任务**（除非需求极其复杂且用户明确同意）',
    '- 如果按规则拆分后 > 20 个 → **必须合并相关任务**，而不是继续拆分',
    '',
    '**📌 核心原则：一个功能章节 = 一个任务**',
    '- 需求文档中的一个功能章节（Section）→ 默认对应一个原子任务',
    '- 只有当该章节明显包含多个独立子模块时，才考虑拆分',
    '- 例如："用户管理"章节 → 一个任务（包含 CRUD + 页面 + 接口）',
    '- 反例：把 "用户管理的增删改查" 拆成 4 个任务 → ❌ 过度拆分',
    '',
    '**合并优先级（从高到低）:**',
    ...
    '',
    '**过度拆分的危害:**',
    '- 管理成本爆炸（56 个任务 = 56 次确认 + 56 个目录 + 56 套 Spec）',
    '- 依赖关系复杂化（A→B→C→D... 链条过长）',
    '- 开发者认知负担加重（需要在多个 Task 间切换上下文）',
    '- 违背 SpecCore "原子任务" 初衷（应该是独立工作单元，不是细碎步骤）',
    '',
    '### 🤖 自动模式指令',
    '',
    '本拆分在自动模式下执行，请遵循以下原则:',
    ...
    '5. **强制数量检查** — 生成 JSON 前自检：如果任务数 > 20，立即重新合并，直到 ≤ 20',
    '',
  ].join('\n');
}
