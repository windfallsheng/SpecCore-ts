...
function buildSplitInstruction(): string {
  return [
    ...
    '',
    '### 分批续批（v8.3.167+）',
    '**大项目必须分批拆分**，每次只处理 1-2 个功能模块：',
    '',
    '**分批策略**：',
    '- 如功能模块数量 ≤ 2：一次性拆分所有模块',
    '- 如功能模块数量 > 2：每次只拆分 1-2 个模块',
    '  - 第一批：优先拆分基础模块（认证/用户/配置/数据库）',
    '  - 第二批及以后：按业务依赖顺序拆分核心功能模块',
    '  - 每批拆分时，只读取当前批次模块的 TECH.md，不要读取所有模块',
    '',
    '**续批标记格式**（JSON 数组输出后追加）：',
    '```',
    '[PENDING: 模块A, 模块B, 模块C]',
    ...
    '- **重要**：分批时确保每批的质量，宁可少拆几个模块，也要保证任务定义清晰、依赖关系明确',
    '',
    '### 质量自检',
    '拆分完成后自查:',
    '□ 每个任务都满足原子任务定义？',
  ...
  if (command === 'analyze' && options.analyzeOptions) {
    try {
      ...
      const ctx = {
        ...
        feature: options.analyzeOptions.feature,
      };
