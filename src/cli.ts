...
program
  ...
  .option("--platform <platform>", "导出指定平台: <端名>（工程标识，从 CONSTITUTION.md 端列表读取）")
...
taskCmd
  ...
  .option('--platforms <platforms>', '指定平台列表，逗号分隔（如 <端名1>,<端名2>）')
  .option('--backend-only', '仅创建后端端（自动识别项目中的后端端名）')
  .option('--frontend-only', '仅创建前端端（自动识别项目中的前端端名）')
  .option('-i, --iteration <iteration>', '目标迭代')
...
program
  ...
  .option('--backend', '仅后端任务（自动识别项目中的后端端名）')
  .option('--frontend', '仅前端任务（自动识别项目中的前端端名）')
  .option('--platform <platform>', '按端过滤（工程标识，如 CONSTITUTION.md 端列表中的端名）')
...
program
  ...
  .option('--platform <platforms>', '评审指定端，逗号分隔多端（如 <端名1>,<端名2>）')
...
program
  ...
  .option('-t, --tech <target>', 'Target tech-stack (TECH_STACK.md): <端名>')
...
program
  ...
  .option('-p, --platform <name>', '平台标识（工程标识，如 CONSTITUTION.md 端列表中的端名）')
