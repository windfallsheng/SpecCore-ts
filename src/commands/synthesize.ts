...
function buildPhase1Prompt(
  ...
  p += `## 分析要求（业内专业级）\n\n`;
  p += `对每个端独立分析，必须覆盖以下 **10 大维度**：\n\n`;
  p += `### 1. 功能清单与用户故事\n`;
  p += `- 功能列表（含优先级 P0/P1/P2）\n`;
  p += `- 每个功能的用户故事：As a {角色}, I want {操作}, so that {价值}\n`;
  p += `- 功能边界说明（做什么、不做什么）\n\n`;
  p += `### 2. 接口定义（API 规格）\n`;
  p += `- RESTful 接口表：方法 | 路径 | 请求参数 | 响应结构 | 状态码 | 说明\n`;
  p += `- 请求/响应示例（JSON）\n`;
  p += `- 错误码定义（按模块划分段）\n`;
  p += `- 分页/排序/过滤约定\n\n`;
  p += `### 3. 数据模型\n`;
  p += `- 实体关系图（ER 描述）\n`;
  p += `- 字段级定义：字段名 | 类型 | 长度 | 必填 | 默认值 | 约束 | 说明\n`;
  p += `- 索引策略（主键/唯一/普通/复合）\n`;
  p += `- 枚举值定义\n\n`;
  p += `### 4. 业务规则\n`;
  p += `- 核心业务逻辑（含边界条件和异常流）\n`;
  ...
  p += `- 数据校验规则（前端 + 后端双重校验）\n\n`;
  p += `### 5. 安全分析\n`;
  p += `- 认证方式（JWT/OAuth/Session）\n`;
  p += `- 授权模型（RBAC/ABAC）\n`;
  p += `- 敏感数据处理（脱敏/加密/日志屏蔽）\n`;
  p += `- 常见防护（XSS/CSRF/SQL注入/越权访问）\n\n`;
  p += `### 6. 性能特征\n`;
  p += `- 预估 QPS / 并发量\n`;
  p += `- 慢操作识别（批量导入/报表/搜索）\n`;
  p += `- 缓存策略（哪些数据缓存/失效策略）\n`;
