...
async function createToolIntegrations(projectRoot: string): Promise<void> {
  const commands: [string, string, string][] = [
    ...
    ['spec-doc2spec', '导入需求: 文件=${1:PRD.docx} 期次=${2:Q1}', 'speccore doc2spec -f ${1:PRD.docx} --iter ${2:Q1}'],
    ['spec-analyze', '需求分析: 期次=${1:Q1} 任务=${2:Task-001}', 'speccore analyze -I ${1:Q1} --task ${2:Task-001}'],
    ['spec-split', '拆分任务: 期次=${1:Q1} 责任人=${2:张三}', 'speccore iteration split -i ${1:Q1} --owner ${2:张三}'],
    ['spec-execute', '执行任务: 期次=${1:Q1} 任务=${2:Task-001} 类型=${3:feature} 端=${4:--backend}', 'speccore execute -i ${1:Q1} -t ${2:Task-001} --type ${3:feature} ${4:--backend} --force'],
    ['spec-plan', '执行计划: 期次=${1:Q1} 责任人=${2:张三}', 'speccore plan -I ${1:Q1} --owner ${2:张三}'],
    ['spec-pr', '创建PR: 任务=${1:Task-001}', 'speccore pr --task=${1:Task-001}'],
    ['spec-done', '任务归档: 任务=${1:Task-001}', 'speccore done --task=${1:Task-001}'],
    ['spec-spec2doc', '导出文档: 期次=${1:Q1} 输出=${2:需求.docx}', 'speccore spec2doc -i ${1:Q1} -o ${2:需求.docx}'],
    ['spec-dev', '智能级联: --auto全自动 --from/--to指定阶段', 'speccore dev'],
    ['spec-change', '需求变更: 描述=${1:变更描述} 任务=${2:Task-001}', 'speccore change "${1:变更描述}" --task=${2:Task-001}'],
    ...
    ['spec-create-iteration', '创建迭代: 名称=${1:Q2} 负责人=${2:张三}', 'speccore iteration create -n ${1:Q2} --owner=${2:张三}'],
    ['spec-retro', '回顾报告: 任务=${1:Task-001}', 'speccore retro --task ${1:Task-001}'],
