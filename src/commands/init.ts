...
当用户在对话中请求修改 Spec 文件时，AI 必须执行以下流程：

### 1. 变更确认流程
- **预览**: 展示将要修改的文件路径和具体内容摘要
- **确认**: 等待用户确认后再写入文件
- **禁止**: 未经确认直接修改 Spec 文件

### 2. 自动追加变更履历
每个被修改的 Spec 文件末尾必须追加：

| 时间 | 变更内容 | 类型 |
| :--- | :--- | :--- |
| 2026-07-21 | + POST /api/v1/orders/batch | 新增接口 |
| 2026-07-20 | 初始创建 | 初始化 |

### 3. 影响分析
- 检查 PROJECT_GRAPH.md 中的依赖关系
- 列出可能受影响的下游任务

### 4. 下一步提示
- 需要重新执行的命令（execute / sync / validate）
