...
async function createToolIntegrations(projectRoot: string): Promise<void> {
  const commands: [string, string, string][] = [
    ...
    ['spec-analyze', '需求分析: 期次=${1:Q1} 任务=${2:Task-001}, 'speccore analyze -I ${1:Q1}'],
    ['spec-split', '智能拆分: 期次=${1:Q1} 责任人=${2:张三}, 'speccore iteration split -i ${1:Q1} --interactive'],
    ['spec-execute', '执行任务: 期次=${1:Q1} 任务=${2:Task-001} 类型=${3:feature}, 'speccore execute -i ${1:Q1} -t ${2:Task-001} --type ${3:feature} ${1:Task-001} --force'],
    ['spec-plan', '执行计划: 期次=${1:Q1} 责任人=${2:张三}, 'speccore plan -I ${1:Q1}'],
