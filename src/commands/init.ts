...
async function createToolIntegrations(projectRoot: string): Promise<void> {
  const commands: [string, string, string][] = [
    ...
    ['spec-split', '拆分任务: 期次=${1:Q1} 责任人=${2|张三,李四,王五|}', 'speccore iteration split -i ${1:Q1} --owner ${2|张三,李四,王五|}'],
    ['spec-execute', '执行任务: 期次=${1:Q1} 任务=${2:Task-001} 类型=${3|feature,bugfix,research|} 端=${4|--backend,--frontend|}', 'speccore execute -i ${1:Q1} -t ${2:Task-001} --type ${3|feature,bugfix,research|} ${4|--backend,--frontend|} --force'],
    ['spec-plan', '执行计划: 期次=${1:Q1} 责任人=${2|张三,李四,王五|}', 'speccore plan -I ${1:Q1} --owner ${2|张三,李四,王五|}'],
    ['spec-pr', '创建PR: 任务=${1:Task-001}', 'speccore pr --task=${1:Task-001}'],
    ['spec-done', '任务归档: 任务=${1:Task-001}', 'speccore done --task=${1:Task-001}'],
    ['spec-spec2doc', '导出文档: 期次=${1:Q1} 格式=${2|需求.docx,方案.pdf,报告.html|}', 'speccore spec2doc -i ${1:Q1} -o ${2|需求.docx,方案.pdf,报告.html|}'],
    ['spec-dev', '智能级联: 期次=${1:Q1} 模式=${2|引导页,--auto全自动|}', 'speccore dev -i ${1:Q1} ${2|,--auto|}'],
    ...
    ['spec-retro', '回顾报告: 任务=${1:Task-001} 格式=${2|总结,详细,图表|}', 'speccore retro --task ${1:Task-001} --format ${2|summary,full,chart|}'],
