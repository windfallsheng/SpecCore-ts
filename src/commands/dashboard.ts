...
export async function dashboardCommand(options: DashboardOptions): Promise<void> {
  ...
  try {
    ...
    // 生成 HTML
    const projectName = (index as any).projectName || 'SPECCORE';
...
@keyframes titleGlow{0%,100%{filter:drop-shadow(0 0 20px rgba(14,165,233,.4)) drop-shadow(0 0 60px rgba(14,165,233,.15))}50%{filter:drop-shadow(0 0 30px rgba(14,165,233,.7)) drop-shadow(0 0 80px rgba(14,165,233,.3))}@keyframes cardGlow{0%,100%{opacity:.5;transform:scale(1)}50%{opacity:1;transform:scale(1.6)}}
...
.header-left h1{font-family:'Orbitron',sans-serif;font-size:26px;font-weight:900;background:linear-gradient(135deg,var(--cyan),var(--purple));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:2px;animation:titleGlow 3s ease-in-out infinite}
...
<div class="stats">
  <div class="stat-card">
    ...
    <div class="data-stream"><span>SPECCORE · TOTAL REQUIREMENTS · ${now} · V${(() => { try { return require(join(process.cwd(), 'package.json')).version; } catch { return '5.71.0'; } })()}</span></div>
  </div>
  <div class="stat-card">
    <button class="fs-btn" title="全屏 (F)" onclick="toggleFS(this.parentElement)">⛶</button>
