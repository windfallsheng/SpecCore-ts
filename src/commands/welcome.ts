...
export async function welcomeCommand(_options: WelcomeOptions): Promise<void> {
  ...
  if (isAiContext() || !process.stdout.isTTY || _options.web) {
    ...
    let phase = 'doc';
    if (iterName) {
      const reqDoc = join(`Iteration-${iterName}`, '020-需求文档', 'REQUIREMENT.md');
      if (!(await pathExists(reqDoc))) phase = 'doc';
      else if (!(await pathExists(join(`Iteration-${iterName}`, '020-需求文档', 'ANALYSIS.md')))) phase = 'analyze';
  ...
  if (!iterName) {
    logger.info(box('📦 项目状态', [`${C.gray}无活跃期次${C.r}`, `${C.b}speccore iteration create -n Q1${C.r}`]));
    logger.info('');
    showAskGuide();
    return;
  }
