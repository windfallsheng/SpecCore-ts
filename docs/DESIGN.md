...
## 附录：v8.3.161+ 大项目分批调度设计

### 设计背景

SpecCore 在 5+ 端、10+ 功能模块的大项目中，全局分析一次性生成所有文档时 Prompt 超出上下文窗口（>32K tokens），导致：
1. **质量稀释**: AI 被迫生成空壳文档（<30 行、无代码引用、无 Mermaid 图）
2. **上下文溢出**: 结构化数据 + 源码索引 + 需求文档同时注入，触发被动裁剪
3. **不可恢复**: 单轮失败后需从头开始，已生成的文档无法复用

### 核心原则

**宁可少分析几个模块，也要保证每个模块的分析质量。**
...
┌─────────────────────────────────────────────────────────────┐
│  Layer 1: 索引扫描（按端分批）                                 │
│  - 端 >5 时，一次处理 3-5 个端的 _INDEX.md                    │
│  - 21 个扫描维度 → 路径引用（Read platforms/{端}/_INDEX.md）   │
├─────────────────────────────────────────────────────────────┤
│  Layer 2: 跨端关联（精简维度）                                 │
│  - 21 项关联维度 → 7 项核心表格                                │
│  - 输入: structured-data.json + Layer 1 索引交叉验证          │
├─────────────────────────────────────────────────────────────┤
│  Layer 3: 模块深入（按功能模块分批）                           │
│  - Light Path: Prompt 指导 AI 一次处理 1-2 个模块              │
│  - Full Path: CLI 检测 pendingModules，自动续批 Layer 3        │
├─────────────────────────────────────────────────────────────┤
│  Layer 4d: 端级技术文档（按端分批）                            │
│  - 一次处理 1-2 个端的所有缺失文档                             │
│  - 自动输出 [SPECCORE_EXEC] 续批                              │
└─────────────────────────────────────────────────────────────┘
```

### 数据层优化：structured-data.json 平台分段

**问题**: 全量结构化数据（所有端的 API/Entity/DTO/Service）单文件可达 5K-10K tokens。

**方案**:
- 写入全量 `structured-data.json`（兼容性保留）
- 额外写入 `structured-data.{platform}.json`（每个端一份，只含该端 API + 关联 DTO/Service）
- `prompt-builder.ts` 优先读取平台分段文件

### 迭代级分批

| 环节 | 分批粒度 | 机制 |
|------|---------|------|
| **analyze Phase 2** | 功能模块 | >2 模块时一次 1-2 个模块的所有端文档 |
| **split** | 功能模块 | >2 模块时一次拆分 1-2 个模块的任务 |
| **plan** | 任务数量 | 全量任务概要（依赖拓扑）+ 前 15 个任务详细文档 |
| **execute** | 源码上下文 | 端 >=3 时自动启用 `--with-code` |

### 其他命令 Pipeline 扩展

**Ask 引擎复合意图识别**:
- 新增 4 个工作流：`analyze-split`、`analyze-split-plan`、`plan-execute`、`analyze-plan-execute`
- 得分系统新增 `analyzeSplit`、`planExecute` 维度
- 支持自然语言描述多步骤意图（如"分析需求然后拆分任务"）

### 效果

| 指标 | 改造前 | 改造后 |
|:---|:---|:---|
| Layer 1 Prompt 大小 | 5K-8K tokens | 2K-3K tokens |
| Layer 3 单次上下文 | 20K+ tokens（所有模块） | 8K-12K tokens（1-2 个模块） |
