...
`--schedule=night` 只是把任务标为 `queue` 状态，不会自动运行。需要手动执行。
...
# 任何时候：手动触发所有 queue 任务
speccore execute --all --scheduled
```

> `--scheduled` 的意思是"只执行 queue 状态的任务"，不需要守护进程。

### 机制二：精确时间调度（指定时间，自动执行）

用 `schedule create --at` 指定精确时间，`schedule daemon start` 后自动执行。
...
# 启动守护进程（只需一次）
speccore schedule daemon start       # 后台运行，到点自动执行
```
