...
| 命令 | 说明 |
| :--- | :--- |
| `speccore init` | 初始化项目（创建目录结构 + WorkBuddy 集成） |
| `speccore import` | 导入项目到全量层 |
| `speccore new-task` | 创建多平台 Task |
| `speccore platform-add` | 动态添加平台 |
| `speccore iteration create` | 创建期次 |
| `speccore progress` | 进度查看 |
| `speccore execute` | 执行任务 |
| `speccore validate` | 合规性检查 |
| `speccore context` | 查看 Task 上下文 |
