...
> 置信度: {EXTRACTED|INFERRED}  ← EXTRACTED=从源码明确提取，INFERRED=分析推断
...
| 标签 | 含义 | 使用场景 |
|:---|:---|:---|
| **EXTRACTED** | 从源码中明确提取 | 直接从代码结构、接口定义、配置文件中观察到的模式 |
| **INFERRED** | 分析推断得出 | 通过多文件关联分析、社区检测、依赖推理得出的模式 |

## 更新规则

- **追加不覆盖**: 同一模式的新变体在文件末尾追加，用 `---` 分隔
- **手动编辑允许**: 支持人工补充、修正、合并
- **自动发现**: 全局分析时自动从源码中识别并生成（默认标记为 EXTRACTED）
- **推断模式需复核**: INFERRED 标记的模式建议人工复核后转为 EXTRACTED
