...
async function initCommand(options) {
    ...
    await doInit(process.cwd(), options, new logger_1.Spinner('Initializing SpecCore'));
}
async function doInit(projectRoot, options, spinner) {
    spinner.start();
    try {
        ...
        logger_1.logger.info('  3. Run: speccore analyze --scope global to analyze your codebase');
        logger_1.logger.info('  4. Run: speccore iteration create --name=Q1 to start an iteration');
        logger_1.logger.info('  5. Run: speccore doc2spec -f requirements.docx to import docs');
        logger_1.logger.info('');
        logger_1.logger.info('💡 WorkBuddy Integration: .workbuddy/ files created.');
        logger_1.logger.info('   Reopen this project in WorkBuddy to enable Speccore commands.');
    }
    catch (error) {
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(speccoreDir, 'RULES', 'POST_COMPLETION.md'), `# Feature 完成后的维护流程
 ...
 */
async function createGlobalFiles(speccoreDir) {
    const globalDir = (0, path_1.join)(speccoreDir, 'GLOBAL');
    // GLOBAL/INDEX.md - 全量需求索引
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'INDEX.md'), `# 全量需求索引（Global Catalog）
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'OVERVIEW.md'), `# 全量项目全景
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'ARCHITECTURE.md'), `# 全量技术架构
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'TECH_STACK.md'), `# 全量技术栈
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'PROTOTYPE_INDEX.md'), `# 全量原型索引
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'GLOSSARY.md'), `# 全量术语表
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'CHANGELOG.md'), `# 全量变更日志
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'PROJECTS', '_template', 'REQUIREMENT.md'), `# {项目名称} - 需求文档
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'PROJECTS', '_template', 'METADATA.md'), `# {项目名称} - 元数据
...
`);
    ...
    await (0, fs_extra_1.writeFile)((0, path_1.join)(globalDir, 'BASELINES', 'README.md'), `# 基线索引
 ...
 */
async function createWorkBuddyFiles(projectRoot) {
    const workbuddyDir = (0, path_1.join)(projectRoot, '.workbuddy');
    // .workbuddy/memory/MEMORY.md — 项目记忆（告诉 WorkBuddy 这是 Speccore 项目）
    await (0, fs_extra_1.ensureDir)((0, path_1.join)(workbuddyDir, 'memory'));
    await (0, fs_extra_1.writeFile)((0, path_1.join)(workbuddyDir, 'memory', 'MEMORY.md'), `# Project Memory — Speccore Managed
...
`);
    ...
    await (0, fs_extra_1.ensureDir)((0, path_1.join)(workbuddyDir, 'skills', 'speccore'));
    await (0, fs_extra_1.writeFile)((0, path_1.join)(workbuddyDir, 'skills', 'speccore', 'SKILL.md'), `---
name: speccore
...
// ── 交互式初始化 ──
async function interactiveInitFlow(options) {
    ...
    logger_1.logger.info(`   模式: ${options.mode === 'full' ? '全量 (51命令)' : '简洁 (17命令)'}`);
