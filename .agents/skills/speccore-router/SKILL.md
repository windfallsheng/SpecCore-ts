...
description: >
  SpecCore CLI 命令路由器。仅在用户明确提到 speccore、迭代、期次、Spec 等关键词，
  或明确要求通过 speccore 命令行工具执行操作，且意图涉及以下概念时使用：
  迭代管理、需求文档导入（doc2spec）、Spec 分析生成（analyze --prompt）、
  任务拆分（split）、执行计划（plan --prompt）、代码执行（execute --prompt）、
  任务归档（done）、PR 提交（pr）、状态查看（status/dashboard/doctor）、智能级联（dev）。
  ...
  模糊意图拦截：如果用户没有明确提到 speccore/迭代/期次/Spec 等关键词，
  只说"分析需求"、"拆分任务"、"做需求分析"等泛泛说法，必须先追问确认：
  "你是指用 speccore 分析迭代需求，还是一般性地讨论需求？"，不可直接执行。
...
当用户说"重新/re-analyze/再分析/重跑"：有合理理由可以确认后再执行，没理由直接执行。禁止检查时间戳判断"无变更"并以此拒绝。确认≠拒绝。
## 🚫 禁止
...
在匹配映射表之前，先判断当前是否有 speccore 上下文，再决定是否拦截：

### 1. 有 speccore 上下文（放行）

如果当前对话中用户**之前已经明确使用过 speccore**（如执行过 analyze/split/execute 等命令，或提到过迭代/期次/Task 等），则视为有上下文：
- ✅ 模糊意图（如"分析需求"、"拆分任务"）**直接按 speccore 处理**，无需重复确认
- 除非用户明确说"这次不用 speccore"、"不用 speccore 分析"、"正常讨论就行"等，才按正常 AI 对话处理

### 2. 无 speccore 上下文（必须确认）

新会话或用户从未提到过 speccore 相关概念时，检查用户输入是否包含以下关键词：

**直接放行（满足任一）**：
- `speccore`、`SpecCore`、`spec-core`
- `迭代`、`期次`、`Iteration-`、`Sprint-`
- `Task-`、`任务号`
- `010-requirements`、`020-specs`、`030-tasks`
- `.speccore/` 目录相关操作
- `/analyze`、`/split`、`/execute` 等快捷命令

**必须追问确认（无上述关键词）**：
| 用户说法 | 处理方式 |
| :--- | :--- |
| "分析需求" | ❌ 不直接执行。追问："你是指用 speccore 分析迭代需求，还是一般性地讨论需求？" |
| "拆分任务" | ❌ 不直接执行。追问："你是指用 speccore 拆分迭代任务，还是一般性地拆解工作？" |
| "做需求分析" | ❌ 不直接执行。追问确认 |
| "评审需求" | ❌ 不直接执行。追问确认 |
| "加个登录功能" | ❌ 不直接执行。追问："你是指用 speccore 创建迭代任务，还是一般性地讨论功能？" |

**追问后的分支处理**：
- 用户回答"用 speccore" / "走 speccore" / "迭代分析" → ✅ 进入 speccore 流程
- 用户回答"不用" / "正常讨论" / "不是" / "一般性讨论" → ❌ 不走 speccore，按正常 AI 对话处理
- 用户回答含糊 → 再次确认，不猜测意图
