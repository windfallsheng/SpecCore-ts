...
**Ask 引擎 review 意图识别增强** (`src/core/ask-engine.ts`):
- 扩展正则覆盖更多自然语言表达：
  - 全局分析/文档：`全局分析不好`、`全局文档有问题`
  - 任务层：`任务文档`、`Task-001 的 spec`
  - 源码/需求检查：`检查源码`、`重新澄清需求`、`完善分析`
  - 质量相关：`分析不好`、`分析有问题`、`文档质量差`
- 参数提取增强：
  - 迭代名：支持直接出现的 `Iteration-001`（无需"迭代"前缀）
  - 多端：支持中文逗号分隔 `backend，frontend`
  - 文档路径：支持 `020-specs/...`、`TECH.md`、`ANALYSIS.md` 等自动识别
  - 全局标志：识别 `全局`、`global` 关键词
  - 任务ID：识别 `Task-xxx` 格式
...
**`speccore init --update` 升级流程卡死** (`src/commands/update.ts`):
- **根因**: `spinner.start()` 在升级开始时启动，但在交互确认 (`askConfirm`) 前没有停止，旋转动画覆盖了输入提示 `[y/N]: `
- **修复**: 在两处 `askConfirm` 调用前添加 `spinner.stop()`，确保用户能看到并输入确认
- **影响**: 升级检测到结构性变更时（如 `quality_gates.commands` 删除），不再出现"一直转圈"的假象

---

## v8.3.139 (2026-09-11) — 本地文件读取增强 + 深度外链支持

### 改进

**本地文件（HTML/图片/SVG）读取策略优化** (`src/core/prompt-builder.ts`):
- HTML 原型文件使用独立阈值 **8KB**（之前 1.5KB），避免大多数原型文件被过度截断
- Markdown/文本文件阈值从 1.2KB 提高到 **2KB**
- 超过阈值的 HTML 文件不再截断 inline（避免结构残缺误导 AI），而是明确提示宿主 AI 直接打开查看
- 超过阈值的文本文件截断后附加明确提示：`📄 **文件内容已截断**（宿主 AI 请务必打开查看完整内容）`
- 本地 PNG/JPG（无视觉模型时）、SVG（过大时）、Vision 描述失败/异常时，统一使用 `🖼️ **图片文件**... ⚠️ **请务必直接打开查看**` 格式

**深度外链支持** (`src/core/prompt-builder.ts`):
- `expandMarkdownLinks` 递归深度从 2 层提高到 **3 层**，支持文档 A → B → C 三级展开
- 三处 `processMarkdownContent` 调用统一更新 `maxLinkDepth: 3`
- 外链资源去重（Map 按 URL 去重），避免重复收集

**外链资源清单 Bug 修复**:
- `extractMarkdownLinks` 原代码提前过滤所有 http/https 外链，导致外链资源清单永远无法生成
- 修复后：外链链接正常收集到 `📎 **外链资源清单**` 和 `🖼️ **外链图片清单**`
...
**`BUSINESS-RULES-EXAMPLE.md` 模板扩展** (`src/commands/init.ts`)：
- 新增「7. 扩展规则（按需补充）」章节，包含 5 大补充项：
  - **7.1 行业特有规则** — 电商（库存扣减/秒杀/优惠券）、医疗（隐私等级/审计/复核）、金融（交易流水/对账/二次确认）
  - **7.2 第三方接口约束** — 支付/短信/邮件/地图/OCR 的限流策略与应对措施
  - **7.3 数据归档策略** — 订单/日志/审计/附件的在线保留期、归档方式、归档后保留期
  - **7.4 多租户规则** — 数据隔离、资源配额（免费/专业/企业版）、租户隔离校验、数据导出
  - **7.5 合规要求** — 等保 2.0、GDPR、PCI-DSS、个人信息保护法
...
**`split` 生成的子任务 `.meta/git-config` 文件格式升级**：

```
# 子任务级 Git 配置（booking-service）
# ─────────────────────────────────────────────────────────────────────────────
...
**改进点**：
- 新格式将 `分支类型` 放在最前面，直观展示当前任务的分支类型
- `前缀` 和 `后缀` 字段以注释形式展示，用户取消注释即可使用
- 增加完整的分支名格式说明和示例
- `源分支`、`分支前缀`、`分支格式`、`自动拉取`、`远程名称` 移至"高级配置"区
- 保护分支改为注释形式列出
...
**CONSTITUTION.md 字段对照总表（v8.3.110）**:
- 新增「## 字段对照总表」章节，位于文档最开头（端列表之前）
- 包含「AI 必读」提示，明确 CONSTITUTION.md 各列与 PROJECT.yaml 字段的对应关系
- 项目信息表字段：8 列完整对照（CONSTITUTION 列 → yaml 字段 → 语义说明 → 示例）
- Git 配置表字段：4 列完整对照
- **特别提醒 AI**：「涉及需求端」的值是需求文档名，不是技术端标识

**PROJECT.yaml 注释更新**:
- `requirement_unit` 注释更新为："涉及需求端：需求文档名称/标识（如'订单系统PRD'），仅作 AI 参考，不做校验"

### 设计意图
- 防止 AI 将「涉及需求端」误解为技术端映射
- 让 AI 在分析需求时，能正确识别工程与需求文档的关联关系
- 统一 CONSTITUTION.md 和 PROJECT.yaml 的字段语义
...
**「涉及需求端」语义澄清（v8.3.108）**:
- 明确「涉及需求端」列的值是**需求文档名称/标识**，仅作为 AI 参考上下文
- 例如："后台管理端需求文档"、"订单系统PRD"、"移动端功能说明书"
- **不是技术端映射**，不做任何校验，不引用「端列表」中的端名
- 作用是告诉 AI：本工程大概会出现在哪些需求文档中
- `init.ts` 模板示例和注释已更新
- `getProjectPathForPlatform` 注释更新：回退匹配仅用于向后兼容旧格式
...
**Verify 命令支持子目录执行（v8.3.101）**:
- `speccore verify` 现在支持在项目子目录执行（向上查找 `.speccore/`）
- 代码路径、测试配置、报告目录等均基于项目根目录解析，不再误用当前工作目录
- **影响文件**：`src/commands/verify.ts`、`src/core/context.ts`

**AI 端列表识别修复（v8.3.101）**:
- 修复 CONSTITUTION.md 被 `.slice(0, 2000)` 截断导致 AI 看不到「端列表」的问题
- 新增 `extractConstitutionForAI()` 智能提取：优先保留「端列表」和「项目信息」章节
- Prompt 中明确告知 AI：「端列表」章节的「工程标识」列才是权威技术端列表
- 「项目信息」表格的「对应需求端」列只是需求模块映射，不是技术端列表
- **影响文件**：`src/core/ai-context-generator.ts`

**术语统一为「工程标识」（v8.3.101）**:
- 所有注释、提示文本、日志中的「对应端」统一改为「对应需求端」
- 明确区分「技术端列表」（端列表章节）和「需求模块映射」（项目信息表格）
- **影响文件**：`src/core/ai-context-generator.ts`

---

## v8.3.100 (2026-09-09) — Update 命令支持子目录执行 + 友好提示

### 修复

**Update 命令支持子目录执行（v8.3.100）**:
- `speccore init --update` 现在支持在项目子目录执行（向上查找 `.speccore/`）
- 未找到项目根时，提示区分"当前目录未检测到"和"父目录找到"两种情况
- 提示信息更友好：告知用户运行 `speccore init` 或切换到已初始化项目的目录
...
**execute 创建分支时在 speccore 目录执行（v8.3.87）**:
- `git-integration.ts` 中 `createTaskBranch` 所有 `execSync` 未传 `cwd`，git 命令在 `process.cwd()`（speccore 目录）执行
- `detectDefaultBranch` 同样未传 `cwd`，检测的是 speccore 自身仓库的分支
- **修复方案**：
  - `createTaskBranch` 新增 `cwd?: string` 参数，所有 git 操作在指定目录执行
  - `detectDefaultBranch` 新增 `cwd?: string` 参数
  - `execute.ts` `prepareTaskBranch` 自动推断工程目录：
    1. 从 `taskDir` 子目录名推断 platform（如 `api-service`）
    2. 在 `PROJECT.yaml` 中查找该 platform 的 `code_path`
    3. 多平台任务取第一个有 `code_path` 的平台
    4. 回退到 `projectConfig.code_scope[0]`
  - `execute.ts` 起始处检测默认分支时，同样使用 `PROJECT.yaml` 中的第一个 `code_path`
- **影响文件**：`src/core/git-integration.ts`、`src/commands/execute.ts`

## v8.3.86 (2026-09-09) — pipeline 分支四级回退 + speccore 自身仓库检测

### 新增

**pipeline 分支选择四级回退（v8.3.86）**:
- 目标分支优先级：环境平台级 branch > 环境全局 branch > PROJECT.yaml `default_branch` > 子任务 `.meta/git-config` 源分支
- 当环境配置未配 branch 时，自动回退到 PROJECT.yaml 中 platform 的 `default_branch`
- 当 PROJECT.yaml 也未配时，自动从当前迭代下的子任务 `.meta/git-config` 中读取 `源分支` 作为最终回退
- **影响文件**：`src/commands/pipeline.ts`

### 修复

**pipeline 在 speccore 自身仓库中误取分支（v8.3.86）**:
- 当 `platform.code_path` 未配置时，`process.cwd()` 默认指向 speccore 目录，而 speccore 本身也是 git 仓库
- 新增 `isSpeccoreOwnRepo()` 检测：若当前 git 仓库的 remote 包含 `speccore` 或 `ts-cli`，输出警告提示用户配置 `code_path`
- 同时优化错误提示：获取分支失败时，提示检查 `code_path` 配置或在正确目录下运行
- **影响文件**：`src/commands/pipeline.ts`

## v8.3.85 (2026-09-09) — 环境模板补充 Java SSH + systemd 完整示例

### 新增

**Java 后端 SSH + systemd 部署模板（v8.3.85）**:
- 环境配置模板 `update-env-configs.ts` 中，`order-api` 默认示例改为 **SSH + systemd**（之前是 Docker）
- 保留 `order-api-docker` 作为容器化部署的备选示例
- 新增脚本模板：
  - `java-systemd.service`：systemd 服务单元文件模板（含 JVM 参数、自动重启、日志配置）
  - `java-restart.sh`：远程重启脚本（停止→替换 jar→启动→健康检查）
- **完整链路**：
  1. `./mvnw clean package` 打包
  2. `scp` 上传 `target/*.jar` 到 `/opt/services/order-api/`
  3. SSH 执行 `java-restart.sh`：停服务 → 替换 jar → 启动 → 检查状态
- **影响文件**：`src/commands/update-env-configs.ts`

## v8.3.84 (2026-09-09) — ask 自动执行确认不再阻断（AI 上下文通知式确认）

### 修复

**ask autoExec.confirm 在 AI 上下文中阻断执行（v8.3.84）**:
- `ask.ts` 第 277 行：`autoExec.confirm` 为 true 时直接 `return`，导致命令未执行
- 根因：AI 上下文（`isAiContext()`）无 TTY 无法交互等待用户输入，但代码统一 return
- **修复方案**：区分 TTY 交互环境和 AI/非 TTY 环境
  - **纯终端**（`isTTY && !isAiContext()`）：输出确认提示，等待用户输入（保持原行为）
  - **AI 上下文**（`isAiContext()`）：输出 `[SPECCORE_INTENT]` + `[SPECCORE_CONFIRM]` 通知标记后继续执行
- 与 pipeline 模式的确认行为保持一致（通知式确认，不阻断）
- **影响文件**：`src/commands/ask.ts`

## v8.3.83 (2026-09-09) — ask 引擎支持 deploy/verify 自动执行

### 新增

**ask 引擎 deploy/verify 意图自动执行（v8.3.83）**:
- `speccore ask` 识别部署意图时，不再只返回建议文本，而是返回 `autoExec` 可执行命令
- 支持从输入中自动提取环境名（测试→test、线上→production 等）和平台名
- **单一 deploy 意图**：`"部署到测试环境"` → `autoExec: { command: 'deploy', args: '--env test --all' }`
- **单一 verify 意图**：`"跑冒烟测试"` → `autoExec: { command: 'verify', args: '--type smoke' }`
- **组合 pipeline 意图**：`"部署到测试环境并跑冒烟测试"` → `mode: 'pipeline'`，自动执行 deploy → verify 两步
- 所有部署/测试命令默认 `confirm: true`，AI 上下文中会输出 `[SPECCORE_INTENT]` 让用户确认后再执行
- **影响文件**：`src/core/ask-engine.ts`

## v8.3.82 (2026-09-09) — pipeline 支持平台级 branch 覆盖全局分支

### 新增

**平台级 Git 分支覆盖（v8.3.82）**:
- 环境配置 `.speccore/environments/{env}.yaml` 中，各端可单独配置 `branch` 字段
- `pipeline` 执行时优先使用平台级 branch，未配置时回退到全局 `branch`
- 支持场景：前端从 `main` 部署，后端从 `backend-stable` 部署
- **配置示例**：
  ```yaml
  env: production
  branch: main              # ← 全局默认值
  platforms:
    h5-mobile:
      branch: main          # ← 覆盖：前端走 main
    api-service:
      branch: backend-stable # ← 覆盖：后端走独立分支
  ```
- **影响文件**：`src/core/environment-config.ts`、`src/commands/pipeline.ts`、`src/commands/update-env-configs.ts`
...
**任务依赖关系图中文节点 ID 渲染失败（plan-html.ts）**：
- 根因：Mermaid v11 不支持 CJK 字符作为裸节点标识符，代码直接用中文 `task.id` 作为节点 ID 导致连线无法解析
- 将 `graph LR` 升级为 `flowchart LR`（Mermaid v11 推荐语法）
- 为每个任务生成安全的 ASCII 节点 ID（`N0`, `N1`...），建立 `task.id → safeId` 映射
- 节点声明、连线、样式三者分离，消除重复定义问题
- 修复后：中文任务名正常显示为节点标签，依赖连线正确渲染

---

## v8.3.30 (2026-08-31) — Skill 意图拦截 + 上下文感知

### 优化

**模糊意图拦截（speccore-router）**：
- 新增**上下文检测**：判断用户是否已有 speccore 使用历史
- **有上下文**：模糊意图（如"分析需求"、"拆分任务"）直接按 speccore 处理，无需重复确认
- **无上下文**：必须追问确认，不猜测意图
- 追问分支：用户确认"用 speccore" → 进入流程；用户拒绝 → 按正常 AI 对话处理

**Skill 描述收紧**：
- `spec-analyze`：明确只在 speccore 上下文触发，不处理一般性"分析需求"讨论
- `spec-split`：明确只在 speccore 上下文触发，不处理一般性"拆分任务"讨论

---

## v8.3.29 (2026-08-31) — 索引过期提示优化 + 模板日期更新

### 优化

**索引过期提示可操作建议（index-guard.ts）**：
- 过期提示从固定的 `speccore refresh` 升级为**分层精准推荐**（`--code`/`--rag`/`--graph`）
- 新增 **AI 对话方式**：`speccore ask "刷新知识图谱并刷新文档索引"`
- 新增**风险提示**：明确告知"继续执行可能基于旧索引，导致分析结果不准确"
- 覆盖命令：`split`、`execute`、`change`、`analyze`、`done`

### 维护

- 更新 `templates/html/` 下 11 个提示词模板日期：`2026-08-21/15` → `2026-08-31`
...
**`analyze --platform` 支持多段指定** (`analyze.ts`):
- `--platform api,web` 支持逗号分隔的多段指定
- 骨架创建时只创建指定端的目录和文件（不指定时仍创建全部端）
- 模糊匹配每个端名，提示非精确匹配结果
- 过滤不在 CONSTITUTION.md 端列表中的端名，给出警告
- apply 模式下多段指定时，由 AI 返回的路径决定写入位置，避免创建 `api,web/` 错误目录

## v8.3.15 (2026-08-25) — 子任务目录用户自定义文档自动扫描

### 改进

**`loadExtraSpecs` 自动扫描用户自定义文档** (`prompt-builder.ts`):
- 执行开发任务时，自动扫描子任务目录下所有 `.md`/`.yaml`/`.yml`/`.json` 文件
- 扫描范围：
  - `00-specs/` 下白名单之外的文档（如 `00-specs/补充需求.md`）
  - `{platform}/{subtask}/` 下递归扫描所有子目录（如 `api/Task-155-xxx/我的笔记.md`）
- 排除目录：`.meta/`、`.git/`、`node_modules/`、`tests/`、`src/`、`dist/`、`build/`
- 安全限制：单文件 2000 字符、额外文件总上限 8000 字符，自动截断并提示
- **设计原则**：用户只要在子任务目录下放文件，AI 就能读到，充分利用机制进行任务执行
...
**DEV_GUIDE.md 开发指南全面改造** (`spec-skeleton.ts` + `split.ts` + `analyze.ts`)
- 从「泛化的环境/规范/联调文档」改造为「可执行的实现指导」
- **新增 7 大必写章节**：改造范围清单、改造顺序与依赖、关键实现步骤（代码级）、接口契约、每步验证方式、回滚方案、常见坑点
- **质量标准更新**：checklist 和 antiPatterns 全部对齐新结构
- **split 任务级 DEV_GUIDE.md**：回退模板不再是空引用，而是结构化 7 章节骨架
...
**问题**：骨架清单与 DOC_MATRIX 是两处独立维护的文档列表，新增文档时可能只更新了 DOC_MATRIX 而遗漏骨架清单，导致文档永远不会被创建。

**解决**：在骨架全部填充完成后，增加 DOC_MATRIX 交叉校验：
- 用 DOC_MATRIX（权威文档清单）对比实际文件系统
- 发现缺失文档时自动补建骨架文件
- 重新检测骨架进度，触发链式推进让 AI 继续填充
- 同时覆盖 Phase 1（全局文档）和 Phase 2（平台文档）

**影响**：即使骨架清单遗漏了某个文档，交叉校验也能在运行时自动检测并补建，彻底消除“文档永远不生成”的 bug。
...
**L1: 骨架质量标准** — `spec-skeleton.ts`
- 新增 `QUALITY_RUBRICS` 常量：12 种文档类型的专业质量检查清单
- 每种文档定义「必含内容」+「禁止反模式」
- `generateSkeleton()` 自动追加质量标准到骨架文件（AI 填充时可见）
- `buildQualityRubric()` 支持后端/前端差异化标准

**L2: PRD + 前序文档自动注入** — `analyze.ts`
- 逐文档链式模式自动注入 010-requirements/ 的 PRD 原文（≤5000 字符）
- 自动注入前序已填充文档摘要（每篇≤ 1500 字符）
- AI 不再需要主动 Read，上下文自动就绪

**L3: 质量标准随骨架可见** — 由 L1 的 generateSkeleton 自动追加实现

**L4: 填充后自动质量校验** — `analyze.ts` + `spec-skeleton.ts`
- `validateContentQuality()` 检查：最低字数、必含章节、占位符残留、表格数据
- 链式推进时自动校验上一文档，评分 < 80 注入警告到 prompt
- 日志输出质量评分（warn < 60 / info < 80）

**L5: split 子任务质量提升** — `split.ts`
- 填充 prompt 注入 REQ.md/TECH.md 质量标准和前后端契约一致性要求
- 最低 200 字符约束，禁止占位符残留

**L6: 质量门禁扩展至 12 项** — `verify-engine.ts`
- 新增 #11 规格文档质量校验（扫描所有 REQ.md/TECH.md 评分）
- 新增 #12 代码文件非空检查（确保 src/ 有实际代码）
...
v7.4.5 的深度标准是一个笼统的表格（「每份文档至少 3 个章节」），忽略了已有的模板系统：
- 迭代级分析已有内置模板（REQUIREMENT.md/ANALYSIS.md/TECH.md 等都有详细章节结构）
- 用户可通过 `.speccore/templates/{global|iteration|task}/` 放自定义模板
- 全局分析 Layer 4 每种文档都有精确的格式描述

### 修复

- **深度标准改为引用模板系统**: 不再用笼统的「3 个章节」数字，而是明确告诉 AI：文档格式由模板定义（用户模板优先，否则按各 Layer 的文档格式说明）
- **全局分析接入用户模板**: 调用 `loadUserTemplates('global')` 读取 `.speccore/templates/global/` 下的用户模板，注入到全局分析 prompt 中
  - 有用户模板时：AI 必须按模板的章节结构和风格生成文档
  - 无用户模板时：按 Layer 4 各文档的默认格式说明生成
- **Layer 专项要求保留**: 每个 Layer 有特定的质量要求（如 Layer 1 条目必须标注源文件路径，Layer 3 必须引用实际代码）

### 涉及文件

- `src/commands/analyze.ts` — 深度标准从笼统表格改为模板引用；全局分析接入 `loadUserTemplates('global')`
